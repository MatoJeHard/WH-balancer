...
function createMainInterface() {
  ...
  let message_info_cluster_mode = `<p>automatic clusters splits villages by coordinates using k-means</p>
        <p>Tribal Wars groups uses your existing village groups as clusters and balances only inside the selected groups</p>
        <p>if a village exists in more than one selected group it will be used only in the first selected group</p>`;
                ...
                <table id="table_main"  class="scriptTable">
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display:flex;justify-content: center; align-items: center;">
                                <div>
                                    <select id="cluster_mode" class="scriptInput">
                                        <option value="auto">automatic clusters</option>
                                        <option value="groups">Tribal Wars groups</option>
                                    </select>
                                </div>
                                <div><a href="#" onclick="UI.InfoMessage(\`${message_info_cluster_mode}\`,20000)"><img src="https://dsen.innogamescdn.com/asset/dbeaf8db/graphic/questionmark.png" style="width: 13px; height: 13px"/></a></div>
                            </div>
                        </td>
                    </tr>
                    <tr id="tr_nr_clusters">
                    ...
                    <tr id="tr_cluster_groups" hidden>
                        <td>cluster groups</td>
                        <td>
                            <div style="display:flex;flex-direction:column;gap:6px;align-items:stretch;max-width:420px;margin:auto;">
                                <div style="display:flex;gap:4px;align-items:center;">
                                    <input type="text" id="cluster_group_filter" class="scriptInput" placeholder="search groups" style="flex:1;min-width:160px">
                                    <input class="btn evt-confirm-btn btn-confirm-yes" type="button" id="btn_refresh_groups" value="refresh">
                                </div>
                                <div style="display:flex;gap:4px;align-items:center;justify-content:space-between;">
                                    <span id="cluster_groups_count">0 selected</span>
                                    <span>
                                        <input class="btn evt-confirm-btn btn-confirm-yes" type="button" id="btn_select_visible_groups" value="all visible">
                                        <input class="btn evt-confirm-btn btn-confirm-yes" type="button" id="btn_clear_groups" value="clear">
                                    </span>
                                </div>
                                <div id="cluster_groups_selected" style="min-height:18px;text-align:left;font-size:11px;line-height:1.35;opacity:0.9"></div>
                                <div id="cluster_groups_list" class="scriptInput" style="height:130px;overflow:auto;padding:4px;text-align:left"></div>
                            </div>
                        </td>
                    </tr>
  ...
  initializeClusterGroupControls();

  //initialization settings
  if (
    localStorage.getItem(game_data.world + "settings_resources_balancer2") !=
    null
  ) {
    ...
    $(
      "#div_container input[type=number], #div_container input[type=checkbox]",
  ...
  $(
    ...
    let data_localStorage = localStorage.getItem(
      game_data.world + "settings_resources_balancer2",
    );
    // console.log(data)
...
function getClusterModeStorageKey() {
  return game_data.world + "settings_resources_balancer2_cluster_mode";
}

function getClusterGroupsStorageKey() {
  return game_data.world + "settings_resources_balancer2_cluster_groups";
}

function initializeClusterGroupControls() {
  let savedMode = localStorage.getItem(getClusterModeStorageKey()) || "auto";
  $("#cluster_mode").val(savedMode);

  let toggleClusterMode = () => {
    let mode = $("#cluster_mode").val();
    if (mode == "groups") {
      $("#tr_nr_clusters").hide();
      $("#tr_cluster_groups").show();
      populateClusterGroupSelect();
    } else {
      $("#tr_nr_clusters").show();
      $("#tr_cluster_groups").hide();
    }
  };

  $("#cluster_mode").on("change", () => {
    localStorage.setItem(getClusterModeStorageKey(), $("#cluster_mode").val());
    toggleClusterMode();
  });

  $("#cluster_group_filter").on("input", () => {
    filterClusterGroups();
  });

  $("#cluster_groups_list").on("click", ".cluster_group_item", (event) => {
    let item = $(event.currentTarget);
    let selected = item.attr("data-selected") == "true";
    setClusterGroupItemSelected(item, selected == false);
    saveSelectedClusterGroups();
    updateClusterGroupsCount();
  });

  $("#btn_refresh_groups").on("click", () => {
    window.resBalancerGroupCache = null;
    populateClusterGroupSelect(true);
  });

  $("#btn_select_visible_groups").on("click", () => {
    $(".cluster_group_item:visible").each(function () {
      setClusterGroupItemSelected($(this), true);
    });
    saveSelectedClusterGroups();
    updateClusterGroupsCount();
  });

  $("#btn_clear_groups").on("click", () => {
    $(".cluster_group_item").each(function () {
      setClusterGroupItemSelected($(this), false);
    });
    saveSelectedClusterGroups();
    updateClusterGroupsCount();
  });
...
function saveSelectedClusterGroups() {
  let selectedGroupIds = getSelectedClusterGroupIds();
  localStorage.setItem(
    getClusterGroupsStorageKey(),
    JSON.stringify(selectedGroupIds),
  );
}

function getSavedClusterGroupIds() {
  try {
    return JSON.parse(localStorage.getItem(getClusterGroupsStorageKey())) || [];
  } catch (error) {
    return [];
  }
}

function populateClusterGroupSelect(forceRefresh = false) {
  let list = document.getElementById("cluster_groups_list");
  if (list == null) return;
  if (list.children.length > 0 && forceRefresh == false) return;

  let savedGroupIds = getSavedClusterGroupIds();
  let groups = getVillageGroups();
  let existingGroupIds = new Set(groups.map((group) => group.id));

  savedGroupIds.forEach((groupId) => {
    if (existingGroupIds.has(groupId) == false) {
      groups.push({
        id: groupId,
        name: `saved group ${groupId}`,
        missing: true,
      });
    }
  });

  $("#cluster_groups_list").empty();

  if (groups.length == 0) {
    $("#cluster_groups_list").append(
      $("<div></div>")
        .css({ padding: "8px", opacity: "0.7", textAlign: "center" })
  ...
  groups.forEach((group) => {
    let item = $("<button></button>")
      .attr("type", "button")
      .addClass("cluster_group_item")
      .attr("data-group-id", group.id)
      .attr("data-group-name", group.name.toLowerCase())
      .attr("data-group-label", group.name)
      .attr("data-group-search", `${group.id} ${group.name}`.toLowerCase())
      .css({
        display: "flex",
        alignItems: "center",
    ...
    item.append(
      $("<span></span>").text(
        group.missing
          ? `${group.name} (not found in menu)`
          : `${group.name} [${group.id}]`,
      ),
    );
    item.append($("<span></span>").addClass("cluster_group_state"));
    setClusterGroupItemSelected(item, savedGroupIds.includes(group.id));
    $("#cluster_groups_list").append(item);
  });
...
function setClusterGroupItemSelected(item, selected) {
  item.attr("data-selected", selected ? "true" : "false");
  item.find(".cluster_group_state").text(selected ? "selected" : "");
  item.css("background", selected ? backgroundInnerTable : backgroundInput);
}

function filterClusterGroups() {
  let filter = ($("#cluster_group_filter").val() || "").toLowerCase().trim();
  $(".cluster_group_item").each(function () {
    let search = $(this).attr("data-group-search") || "";
    $(this).toggle(search.includes(filter));
  });
  updateClusterGroupsCount();
}

function updateClusterGroupsCount() {
  let selected = $(".cluster_group_item[data-selected='true']").length;
  let visible = $(".cluster_group_item:visible").length;
  let total = $(".cluster_group_item").length;
  $("#cluster_groups_count").text(
    `${selected} selected / ${visible} visible / ${total} total`,
  );
  $("#cluster_groups_selected").html(getSelectedClusterGroupsSummaryHtml());
}

function getVillageGroups() {
  if (
    window.resBalancerGroupCache != null &&
    window.resBalancerGroupCache.length > 0
  ) {
    return JSON.parse(JSON.stringify(window.resBalancerGroupCache));
  }

  let mapGroups = new Map();
  let links = [
    game_data.link_base_pure + "overview_villages&mode=prod",
    game_data.link_base_pure + "overview_villages&mode=prod&group=0",
    game_data.link_base_pure + "groups",
    game_data.link_base_pure + "groups&mode=overview",
  ];

  addVillageGroupsFromDocument(document, mapGroups);
  links.forEach((link) => {
    let dataPage = safeHttpGet(link);
    if (dataPage == null || dataPage == "") return;
    const parser = new DOMParser();
    const htmlDoc = parser.parseFromString(dataPage, "text/html");
    addVillageGroupsFromDocument(htmlDoc, mapGroups);
  });

  let groups = Array.from(mapGroups.values());
  groups.sort((a, b) => a.name.localeCompare(b.name));
  window.resBalancerGroupCache = JSON.parse(JSON.stringify(groups));
  console.log("village groups found", groups);
  return groups;
}

function addVillageGroupsFromDocument(doc, mapGroups) {
  if (doc == null) return;

  Array.from($(doc).find("select option")).forEach((option) => {
    let parentSelect = option.parentElement;
    let selectId = parentSelect == null ? "" : parentSelect.id || "";
    let selectName = parentSelect == null ? "" : parentSelect.name || "";
    let selectData = `${selectId} ${selectName}`.toLowerCase();
    let id = getGroupIdFromValue(option.value);

    if (id != null && selectData.includes("group")) {
      addVillageGroup(mapGroups, id, option.textContent);
    }
  });

  Array.from($(doc).find("a[href*='group='], a[href*='group_id=']")).forEach(
    (link) => {
      let id = getGroupIdFromValue(link.getAttribute("href"));
      if (id != null) {
        addVillageGroup(mapGroups, id, link.textContent);
      }
    },
  );

  Array.from($(doc).find("[data-group-id], [data-group]")).forEach((elem) => {
    let id =
      getGroupIdFromValue(elem.getAttribute("data-group-id")) ||
      getGroupIdFromValue(elem.getAttribute("data-group"));
    if (id != null) {
      addVillageGroup(mapGroups, id, elem.textContent);
    }
  });
}

function addVillageGroup(mapGroups, id, name) {
  name = String(name || "")
    .replace(/\(\d+\)/g, "")
    .replace(/\s+/g, " ")
    .trim();
  ...
  if (ignoredNames.includes(lowerName)) return;
  if (mapGroups.has(id)) {
    let existing = mapGroups.get(id);
    if (existing.name.startsWith("group ")) {
      mapGroups.set(id, { id: id, name: name });
    }
    return;
  }
...
function getGroupIdFromValue(value) {
  if (value == null) return null;
  let trimmedValue = String(value).trim();
  if (trimmedValue.match(/^\d+$/) && parseInt(trimmedValue) > 0) {
    return trimmedValue;
  }

  let match = trimmedValue.match(/(?:^|[?&])(?:group|group_id)=(\d+)/);
...
function getSelectedClusterGroupIds() {
  return Array.from($(".cluster_group_item[data-selected='true']")).map(
    (item) => item.getAttribute("data-group-id"),
  );
}

function getSelectedClusterGroupsSummaryHtml() {
  let selectedItems = Array.from(
    $(".cluster_group_item[data-selected='true']"),
  );
  if (selectedItems.length == 0) return "selected: none";

  let labels = selectedItems.map((item) => {
    let id = item.getAttribute("data-group-id");
    let name = item.getAttribute("data-group-label") || "";
    return `<span style="display:inline-block;margin:1px 4px 1px 0;padding:1px 4px;border:1px solid ${borderColor};background:${backgroundInnerTable};color:${textColor}">${escapeHtml(name)} [${escapeHtml(id)}]</span>`;
  });

  return `selected: ${labels.join("")}`;
}

function safeHttpGet(link) {
  try {
    return httpGet(link);
  } catch (error) {
    console.log("failed to read groups from", link, error);
    return "";
  }
}

function getGroupClusters(selectedGroupIds) {
  let allGroups = getVillageGroups();
  let mapGroupNames = new Map(allGroups.map((group) => [group.id, group.name]));
  let assignedCoords = new Set();
  let clusters = [];

  selectedGroupIds.forEach((groupId) => {
    let coords = getCoordsForVillageGroup(groupId);
    let clusterCoords = [];

    coords.forEach((coord) => {
      if (assignedCoords.has(coord)) {
        console.log(`skip duplicated village ${coord} in group ${groupId}`);
        return;
      }
    ...
    if (clusterCoords.length > 0) {
      clusters.push({
        data: clusterCoords,
        mean: getMeanCoord(clusterCoords),
        name: mapGroupNames.get(groupId) || `group ${groupId}`,
        groupId: groupId,
      });
    }
  });
...
function getCoordsForVillageGroup(groupId) {
  let link =
    game_data.link_base_pure + "overview_villages&mode=prod&group=" + groupId;
  let dataPage = httpGet(link);
  const parser = new DOMParser();
  const htmlDoc = parser.parseFromString(dataPage, "text/html");
  let pages = getPagedOverviewUrls(htmlDoc, link);
  let coords = new Set();

  pages.forEach((page) => {
    let pageData = httpGet(page);
    getCoordsFromProductionPage(pageData).forEach((coord) => coords.add(coord));
  });

  return Array.from(coords);
}

function getPagedOverviewUrls(htmlDoc, defaultLink) {
  let list_pages = [];
  if ($(htmlDoc).find(".paged-nav-item").parent().find("select").length > 0) {
    Array.from(
      $(htmlDoc).find(".paged-nav-item").parent().find("select").find("option"),
    ...
    list_pages.push(defaultLink);
  }
  return list_pages.reverse();
}

function getCoordsFromProductionPage(dataPage) {
  const parser = new DOMParser();
  const htmlDoc = parser.parseFromString(dataPage, "text/html");
  let coords = [];

  if (game_data.device == "desktop") {
    Array.from($(htmlDoc).find(".row_a, .row_b")).forEach((row) => {
      let villageName = $(row).find(".quickedit-vn").text();
      let match = villageName.match(/[0-9]{3}\|[0-9]{3}/);
      if (match != null) coords.push(match[0]);
    });
  } else {
    Array.from(
      $(htmlDoc).find(".overview-container .overview-container-item"),
    ).forEach((row) => {
      let villageName = $(row).find(".quickedit-label").text();
      let match = villageName.match(/\d+\|\d+/);
      if (match != null) coords.push(match[0]);
    });
  }
...
function getClusterLists(
  clusters,
  list_production,
  list_production_home,
  map_incoming,
) {
  ...
  let mapProduction = new Map();
  let mapProductionHome = new Map();

  for (let i = 0; i < list_production.length; i++) {
    mapProduction.set(list_production[i].coord, list_production[i]);
    mapProductionHome.set(
      list_production_home[i].coord,
      list_production_home[i],
    );
  }
  ...
  for (let i = 0; i < clusters.length; i++) {
    ...
    for (let j = 0; j < list_coords.length; j++) {
      let coord = Array.isArray(list_coords[j])
        ? list_coords[j].join("|")
        : list_coords[j];

      if (mapProduction.has(coord) == false) continue;

      let production = mapProduction.get(coord);
      list_prod.push(production);
      list_prod_home.push(mapProductionHome.get(coord));
      ...
      map_draw_on_map.set(production.id, {
        label_cluster: filteredClusters.length,
        villageId: production.id,
  ...
  return {
    ...
    list_production_cluster: list_production_cluster,
    list_production_home_cluster: list_production_home_cluster,
    map_draw_on_map: map_draw_on_map,
  };
}

async function balancingResources() {
  let time_construction_total = parseFloat(
    document.getElementById("time_construction").value,
  );
  let averageFactor = parseFloat(
    document.getElementById("nr_average_factor").value,
  );
  let reserveMerchants = parseInt(
    document.getElementById("nr_merchants_reserve").value,
  );
  let merchantCapacity = parseInt(
  ...
  let clusterMode = document.getElementById("cluster_mode").value;
  let selectedGroupIds = getSelectedClusterGroupIds();

  if (clusterMode == "groups" && selectedGroupIds.length == 0) {
    UI.ErrorMessage("select at least one cluster group", 3000);
    return;
  }

  reserveMerchants =
    Number.isNaN(reserveMerchants) == true || reserveMerchants < 0
      ? 0
      : reserveMerchants; //by default are 0 merchants as reserver
  nrClusters =
    Number.isNaN(nrClusters) == true || nrClusters < 1 ? 1 : nrClusters; //by default is one cluster
  time_construction_total =
    Number.isNaN(time_construction_total) == true || time_construction_total < 0
      ? 20
      : time_construction_total; //if something is not right by default is 20hours
  time_construction_total =
    time_construction_total > 50 ? 50 : time_construction_total; //limit construction time to 50 hours

  averageFactor =
    Number.isNaN(averageFactor) == true
      ? 1
      : averageFactor < 0
        ? 0
        : averageFactor > 1
          ? 1
          : averageFactor; //if>1 is 1 if <0 is 0 if is NaN is 1
  merchantCapacity =
    Number.isNaN(merchantCapacity) == true
      ? 1000
      : merchantCapacity < 1000
        ? 1000
        : merchantCapacity > 1500
          ? 1500
          : merchantCapacity; //capacitiy is 1000 but for .PT might be 1500
  ...
  ///////////////////////////////////////////////get clusters///////////////////////////////////////
  let clusters = [];
  if (clusterMode == "groups") {
    clusters = getGroupClusters(selectedGroupIds);
  } else {
    let kmeans_coords = [];
    for (let i = 0; i < list_production.length; i++) {
      kmeans_coords.push([
    ...
    clusters = getClusters(kmeans_coords, options);
  }
  console.log("clusters", clusters);

  let clusterLists = getClusterLists(
    clusters,
    list_production,
    list_production_home,
    map_incoming,
  );
  clusters = clusterLists.clusters;
  let list_production_cluster = clusterLists.list_production_cluster;
  let list_production_home_cluster = clusterLists.list_production_home_cluster;
  let map_draw_on_map = clusterLists.map_draw_on_map;

  if (clusters.length == 0) {
    UI.ErrorMessage("no villages found in selected cluster groups", 5000);
    createMainInterface();
    changeTheme();
    return;
  }
  ...
  let list_production_balanced = [];
  for (let i = 0; i < list_production_cluster.length; i++) {
    for (let j = 0; j < list_production_cluster[i].length; j++) {
      list_production_balanced.push(list_production_cluster[i][j]);
    }
  }
  console.log("list_production_balanced", list_production_balanced);
  ...
  for (let i = 0; i < list_production_balanced.length; i++) {
    let coord = list_production_balanced[i].coord;
    if (map_incoming.has(coord)) {
      list_production_balanced[i].wood += map_incoming.get(coord).wood;
      list_production_balanced[i].stone += map_incoming.get(coord).stone;
      list_production_balanced[i].iron += map_incoming.get(coord).iron;

      //in case minting a village might have huge cantity of resources underway
      list_production_balanced[i].wood = Math.min(
        list_production_balanced[i].wood,
        list_production_balanced[i].capacity,
      );
      list_production_balanced[i].stone = Math.min(
        list_production_balanced[i].stone,
        list_production_balanced[i].capacity,
      );
      list_production_balanced[i].iron = Math.min(
        list_production_balanced[i].iron,
        list_production_balanced[i].capacity,
      );
    }
    avg_wood_total +=
      list_production_balanced[i].wood / list_production_balanced.length;
    avg_stone_total +=
      list_production_balanced[i].stone / list_production_balanced.length;
    avg_iron_total +=
      list_production_balanced[i].iron / list_production_balanced.length;

    total_wood_home += list_production_balanced[i].wood;
    total_stone_home += list_production_balanced[i].stone;
    total_iron_home += list_production_balanced[i].iron;
  }
  ...
  for (let i = 0; i < list_production_balanced.length; i++) {
    let nr_merchants = 0;
    if (map_nr_merchants.get(list_production_balanced[i].coord))
      nr_merchants = map_nr_merchants.get(list_production_balanced[i].coord);

    // console.log(`coord: ${list_production[i].coord},merchants calculated: ${nr_merchants} vs theory: ${list_production[i].merchants}`)
    list_production_balanced[i].merchantAvailable =
      list_production_balanced[i].merchants - nr_merchants;
  }
  ...
  for (let i = 0; i < list_production_balanced.length; i++) {
    for (let j = 0; j < list_launches.length; j++) {
      if (
        list_production_balanced[i].coord == list_launches[j].coord_destination
      ) {
        list_production_balanced[i].wood += list_launches[j].wood;
        list_production_balanced[i].stone += list_launches[j].stone;
        list_production_balanced[i].iron += list_launches[j].iron;
      } else if (
        list_production_balanced[i].coord == list_launches[j].coord_origin
      ) {
        list_production_balanced[i].wood -= list_launches[j].wood;
        list_production_balanced[i].stone -= list_launches[j].stone;
        list_production_balanced[i].iron -= list_launches[j].iron;
    ...
    list_production_balanced[i].result_wood =
      list_production_balanced[i].wood - Math.round(avg_wood_total);
    list_production_balanced[i].result_stone =
      list_production_balanced[i].stone - Math.round(avg_stone_total);
    list_production_balanced[i].result_iron =
      list_production_balanced[i].iron - Math.round(avg_iron_total);
    list_production_balanced[i].result_total =
      list_production_balanced[i].result_wood +
      list_production_balanced[i].result_stone +
      list_production_balanced[i].result_iron;
  }
  list_production_balanced.sort((o1, o2) => {
...
function calculateLaunches(
  ...
  for (let i = 0; i < list_production_cluster.length; i++) {
    ...
    list_clusters_stats.push({
      ...
      center: clusters[i].name
        ? `${escapeHtml(clusters[i].name)} (${parseInt(clusters[i].mean[0])}|${parseInt(clusters[i].mean[1])})`
  ...
  for (let i = 0; i < list_production.length; i++) {
    ...
    html_end_result += `
        <tr >
            ...
            <td>${formatNumber(Number.isFinite(Number(list_production[i].time_finished)) ? parseInt(Number(list_production[i].time_finished) * 10) / 10 : 0)}</td>
  ...
  if (localStorage.getItem(game_data.world + "constantBuildings") !== null) {
    ...
    for (let i = 0; i < list_buildings.length; i++) {
      ...
      let build_time_factor = Number(
        list_buildings[i].getElementsByTagName("build_time_factor")[0]
          .innerText,
      );
